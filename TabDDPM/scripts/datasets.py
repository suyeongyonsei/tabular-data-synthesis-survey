...
DATA_DIR = Path('/content/drive/MyDrive/ITRC/tab-ddpm/data/carInsurance/carInsurance.csv')
SEED = 0
...
def Insurance():
    # Get the file here: https://www.kaggle.com/shrutimechlearn/churn-modelling
    dataset_dir = Path('/content/drive/MyDrive/ITRC/tab-ddpm/data/carInsurance')
    df = pd.read_csv('/content/drive/MyDrive/ITRC/tab-ddpm/data/carInsurance/carInsurance.csv')

    #df = df.drop(columns=['RowNumber', 'CustomerId', 'Surname'])
    #df['Gender'] = df['Gender'].astype('category').cat.codes.values.astype(np.int64) category를 숫자로 표현 
    y_all = df.pop('CarInsurance').values.astype(np.int64)
    num_columns = ['Balance','NoOfContacts','PrevAttempts']
    cat_columns = ['Age','Job','Marital','Education','Default','HHInsurance','CarLoan',
...
def wine():
    dataset_dir = Path('/content/drive/MyDrive/ITRC/tab-ddpm/data/wine')

    data = pd.read_csv('/content/drive/MyDrive/ITRC/tab-ddpm/data/wine/wine.csv')
    data = data.loc[:, ~data.columns.str.contains('^Unnamed')]
    data

    X_num_all = data.iloc[:, :-1].values.astype(np.float32)
    y_all = _encode_classification_target(data['class'].values)
    idx = _make_split(len(X_num_all), y_all, 3)
